...
export default function DriverTag({ position, teamColor, short, className }: Props) {
	return (
		<div
			...
			className={clsx("flex w-fit items-center justify-between rounded-lg bg-zinc-500 px-1 py-1 font-black", className)}
			style={{ backgroundColor: `#${teamColor}` }}
		>
			{position && <p className="pl-1 text-xl leading-none">{position}</p>}
